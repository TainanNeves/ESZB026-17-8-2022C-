...
float valorV = 0;                 // V antes da vírgula
...
void enviaDadosPelaSerial(){              // opcional, para debug
  ...
  Serial.print(buffer[5],DEC); Serial.print(" "); // tensão antes da vírgula
  Serial.println(buffer[6],DEC); // tensão depois da vírgula
}
...
void loop(){                                            // Le o valor no potenciometro a cada 1 segundo
  ...
  valorV = (valorPot*5.0/1024.0);
  buffer[5] = valorV;                                   // valor de V antes da vírgula
  buffer[6] = (valorV*100.0)-(buffer[5]*100.0);         // valor de V antes da vírgula
...
void funcaoDadoRecebido(int x){                         // funcao chamada ao receber um dado
  ...
  if( dadoRecebido == 0x10){                             // se for definir o limite do alarme
      buffer[2] = buffer[0];
      buffer[3] = buffer[1];
